...
class Feedaty extends Module
{
	...
	private function fdSetCache($id, $value, $expiration = null)
	{
		...
		if ($count == 0)
			$q = 'INSERT INTO `'._DB_PREFIX_.'feedaty_cache` (`id_key`, `value`, `expiration`) values (\''.pSQL((string)$id).'\',\''.pSQL(str_replace('"','\"',(string)$value),true).'\',\''.pSQL((int)$expiration).'\') ';
		/* If there is already a value we update its content */
		else
			$q = 'UPDATE `'._DB_PREFIX_.'feedaty_cache` SET `value` = \''.pSQL(str_replace('"','\"',(string)$value),true).'\', `expiration` = \''.pSQL((int)$expiration).'\' WHERE `id_key` = \''.pSQL((string)$id).'\'';
		Db::getInstance()->Execute($q);
		return true;
	}
